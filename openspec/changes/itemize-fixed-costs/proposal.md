...
- Compute a **suggested** `fixedCostsPercent` and `fixedCostsMonthly` automatically from `(sum(lineItems) + sum(debts[].minimumPayment)) / totalMonthlyIncome`. Debt minimum payments captured on the earlier Debts step are folded in automatically so users never need to re-enter them. When the user lands on the Spending Plan page, the Fixed Costs slider is pre-seeded to the derived value.

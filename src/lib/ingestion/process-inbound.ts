...
/** Land parsed/extracted rows as DRAFT Events on a stable per-sender
 * source, with the same natural-key dedup as every other import path. */
async function landDrafts(
  userId: string,
  source: {
    name: string;
    kind: "IMPORT_ICS" | "EMAIL_FORWARD";
    categories: string[];
  },
  events: {
    startDate: string;
